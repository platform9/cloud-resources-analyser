...
BIN := cloud-analyser
REPO := cloud-resources-analyser
LDFLAGS := ""
NAME ?= cloud-analyser# default lable for docker build
TAG ?= latest
XDG_CACHE_HOME := /tmp
...
build:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -a -o $(BIN_DIR)/$(BIN) main.go
